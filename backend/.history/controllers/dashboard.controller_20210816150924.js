  ...
  getFriend: (req, res, next) => {
    ...
    jwt.verify(token, serectKey, async (error, data) => {
      if (error) {
        ...
        const list = await user.findById(friendId).populate("messages");
        if (list) {
          const result = {
            id: list._id,
            avatar: list.avatar,
          ...
          return res.status(204).send("Loi khong xac dinh");
        }
      }
    });
  },
  findUser: (req, res, next) => {
    ...
    jwt.verify(token, serectKey, async (error, data) => {
      if (error) {
        ...
        let listInfo = [];
        listUserResult.forEach((value) => {
          const avatar = value.avatar;
          const username = value.username;
          const id = value._id;
          if (id != data.id) listInfo.push({ avatar, username, id });
        });
