...
function LeftBar(props) {
  ...
  const {listFriend, listRoom, findUser} = props;
  ...
  const { deleteMusic, chatNow } = props;
    ...
    <div className="flex flex-col mr-2 left-bar w-1/3 ">
      ...
      <div className="h-3/4 w-full flex ">
        ...
        <div className="w-2/5 h-full bg-gray-300 rounded-xl p-2 flex flex-col"
          id="left-bar-friend0">
            <form onSubmit={handleSubmit(findUser)} className="search-user flex items-center  bg-gray-200 rounded-full mb-4">
              <input type="text" className="h-8 pl-4 outline-none bg-transparent text-2xl w-10/12" placeholder="Tim kiem nguoi dung"  {...register("find_user")}/>
            ...
            <ul className=" friend w-full max-h-40 bg-gray-300 rounded-xl overflow-hidden hover:overflow-y-scroll mb-2 px-4" >
              {
                listFriend.map((value,pos)=>(
                  <li className="friend flex w-full my-3 hover:bg-white p-2 rounded-full" id={value._id} onClick={chatNow}>
                <img src={value.avatar} className="rounded-full w-12 h-12" alt="" />
          ...
          <div className="friend h-auto text-center mb-1 w-5/6 mx-auto bg-white rounded-full">Nhom</div>
            <ul className=" friend w-full max-h-40 bg-gray-300 rounded-xl overflow-hidden hover:overflow-y-scroll px-4" >
            {
                listRoom.map((value,pos)=>(
                  <li className="friend flex w-full my-3 hover:bg-white p-2 rounded-full" id={value._id}>
                <img src={value.avatar} className="rounded-full w-12 h-12" alt="" />
                <div className="self-center mx-2 w-full">
