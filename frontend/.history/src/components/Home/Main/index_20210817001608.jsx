...
function Main(props) {
  ...
  const { socket} = props;
  socket.auth = { username: userInfor.username };
  socket.connect();
  ...
  const sendMessage = (data,e)=>{
    ...
    const messaging = {content: message, time: Date.now(), user: receiver, userOwner: sender};
    let x = [...currentUser.messages];
    x.push(messaging);
    
    setCurrentUser({...currentUser, messages:[...x]});

    let id = listUser.forEach(value=>{if(value.username === currentUser.username) return value.userID});
        ...
        <ul className="chat-show bg-bg_chat w-full rounded-bl-2xl rounded-br-2xl shadow-inner p-5" id="listMessage">
          {currentUser.messages !== undefined?currentUser.messages.map((value, pos)=>(
            ...
            <span className="time-message">
              {new Date(value.time).toLocaleTimeString() + " - " + new Date(value.time).toLocaleDateString('vn')}
            </span>
          </li>:
