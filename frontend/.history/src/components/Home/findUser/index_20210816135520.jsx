...
function FindUser(props) {
    ...
    return (
        <div className="w-full h-screen bg-black fixed" id="findUser">
            <button className="text-white absolute top-2 right-2" onClick={()=>{if(document.getElementById("findUser"))document.getElementById("findUser").style.display = "none";}}>Exit</button>
            <div className="absolute w-3/4 h-3/4 top-1/2 left-1/2 bg-white transform -translate-x-1/2 -translate-y-1/2 rounded-3xl px-4">
                <ul className="px-4 py-4 h-11/12 overflow-hidden hover:overflow-y-scroll  ">
