...
function Home(props) {
  ...
  const handleAddFriend = async (e)=>{
    ...
    // });
    data.status = 200;
    if(data.status === 200) toast.success(`Them ban thanh cong`, {
      position: "top-right",
      autoClose: 1000,
      hideProgressBar: false,
      closeOnClick: true,
  ...
  return (
    <div className="flex flex-start h-screen">
      <LeftBar onSubmit={handleSubmit} deleteMusic={deleteMusic} listFriend={listFriends} listRoom={listRooms} findUser={onFindUser} />
      <Main />
      <ResultSearch searchResult={response} handleExit={handleExit} handleAddLink={onAddLink} />
      <ToastContainer />
      <FindUser listUser={listInfo}/>
