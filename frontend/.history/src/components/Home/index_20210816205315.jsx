...
import { addNewUser } from "../../reduxSlice/userCurrentSlice";
...
function Home(props) {
  ...
  const handleChatNow = async (e)=>{
    ...
    if(id){
      ...
      const userId = id;
      const response = await axiosClient.get(`/dashboard/getfriend?friendId=${userId}`,{
        headers: headers
      });
      const action = addNewUser(response.data);
      dispatch(action);
    }
  }
  ...
  const sendMessage = (data,e)=>{
    ...
    const receiver = currentUser.result.id;
    const message = data.message;
    const newElement = document.createElement('<li className="message message-sender max-w-md">\
    ...
    document.getElementById("listMessage").insertAdjacentElement("beforeend", newElement);
  ...
  return (
    <div className="flex flex-start h-screen">
      ...
      <Main sendMessage={sendMessage} socket={socket}/>
