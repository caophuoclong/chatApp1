  ...
  return (
    <div className="fixed w-full h-screen result" id="result-search-complete">
      <FontAwesomeIcon
        ...
        size="2x"
      />
      <div className="box bg-white rounded-3xl w-3/4 h-3/4 overflow-y-scroll absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 p-12">
      
        <ul className="">
          {searchResult.map((value, pos) => (
            <li
              className="my-4 hover:bg-gray-300 flex justify-start items-center cursor-pointer p-4"
              key={pos}
              id={`lst${pos}`}
